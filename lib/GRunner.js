...
class GRunner {
            ...
            if(Array.isArray(d)) {
                return (__cb) => {
                    let f = d.map(pd => {
                        return (__cb) => {
                            if(Array.isArray(pd)) {
                                let nestedSerFun = _this._depMap(pd, ctx);
                                __.series(nestedSerFun, __cb);
                                return;
                            }
                            _this._runner(pd, __cb, _this._incCtx(ctx, 'p'));
                        };
                    });
                    __.parallel(f, __cb);
                };
    ...
    _runner(taskName, cb, ctx) {
        ...
        let serFun = _this._depMap(task.dep, ctx);
        serFun.push(__cb => {
    ...
    _handleResult(taskName, res, doneCb) {
        ...
        if(typeof res.then === 'function') {
            ...
            let onPipeEnd = throught.obj((o, enc, cb) => { cb(); }, cb => {
                doneCb(); cb();
            } );
            res.pipe(onPipeEnd);
        }
    ...
    _exec(taskName, task, cb, info) {
        ...
        const pad = Array(ctx.depth + 1).join('.');
        ...
        ctx.onDone = resHandler;

        let res = null;
        try {
            res = _this.options.exec
                ? _this.options.exec(doneCb, ctx)
                : taskFun(doneCb, ctx);
        } catch(e) {
            doneCb(e);
            return;
        }
        resHandler(res, doneCb);
    }
